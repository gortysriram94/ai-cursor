...
const USE_CASES = [
  {
    ...
    label: "Generic",
    tagline: "AI assistance for anything",
    actions: ["Reply to any message", "Summarize content", "Rewrite & improve", "Quick Q&A"],
  },
  {
    emoji: "📈",
    ...
    actions: ["Objection handling", "Follow-up drafts", "Proposal polish", "Lead qualification"],
  },
  {
    emoji: "📨",
    ...
    actions: ["Cold email sequences", "LinkedIn messages", "Personalised intros", "A/B subject lines"],
  },
  {
    emoji: "💬",
    gradient: "from-purple-500/20 to-transparent",
    label: "Customer Support",
    tagline: "Faster, empathetic replies",
    actions: ["Ticket responses", "Escalation summaries", "Tone softening", "Policy explanations"],
  },
  {
    emoji: "🛒",
    ...
    actions: ["Product descriptions", "Review replies", "Return templates", "Promo copy"],
  },
  {
    emoji: "</>",
    ...
    actions: ["Code review", "Explain functions", "Write docstrings", "Debug reasoning"],
  },
  {
    emoji: "📊",
    gradient: "from-yellow-500/20 to-transparent",
    label: "Finance",
    tagline: "Insight from numbers",
    actions: ["Variance analysis", "Client reports", "Risk summaries", "Executive briefs"],
  },
  {
    emoji: "🏠",
    gradient: "from-orange-400/20 to-transparent",
    label: "Real Estate",
    tagline: "Win more listings",
    actions: ["Property descriptions", "Lead replies", "Client summaries", "Offer letters"],
  },
  {
    emoji: "✏️",
    gradient: "from-rose-500/20 to-transparent",
    label: "Content",
    tagline: "Create without the blank page",
    actions: ["Social captions", "Thread rewrites", "SEO descriptions", "Repurpose content"],
  },
  {
    emoji: "🏢",
    ...
    actions: ["Internal comms", "Policy drafts", "Meeting summaries", "Status updates"],
  },
  {
    emoji: "🔬",
    ...
    actions: ["Paper summaries", "Methodology review", "Citation extraction", "Key findings"],
  },
];
...
const FAQS = [
  {
    q: "Is it really free?",
    a: "Yes. The app is completely free to download and use. The local AI model (qwen2.5:14b via Ollama) is also free. You only pay if you choose to add a cloud API key for faster responses.",
  },
  {
    q: "Does my data leave my computer?",
    a: "No. By default everything runs locally using Ollama. Your text, history, and style profiles never leave your machine. Cloud mode is opt-in only.",
  },
  {
    q: "What happens on first launch?",
    a: "The app downloads the qwen2.5:14b AI model (~9 GB, one time only). A progress bar shows the download and it resumes if interrupted. After that, it starts instantly every time.",
  },
  {
    q: "Which apps does it work in?",
    a: "Any app on Windows or macOS — Gmail, Outlook, Slack, VS Code, Notion, Chrome, Word, Excel, Linear, Jira, and more. Anything with a text field or selectable text.",
  },
  {
    q: "What are the system requirements?",
    a: "Windows 10/11 (64-bit) or macOS 13+. 16 GB RAM recommended for the 14B model. Apple Silicon Macs (M1/M2/M3/M4) run it especially fast thanks to unified memory.",
  },
  {
    q: "How is this different from ChatGPT?",
    a: "ChatGPT requires switching tabs, copy-pasting, and switching back — every single time. AI Cursor lives in a floating panel triggered by Alt+A. No tab switching, no paste, the result goes directly back into whatever app you're using.",
  },
  {
    q: "Can I use my own API key?",
    a: "Yes. Add your NVIDIA NIM key in Settings for cloud-powered responses. The app automatically falls back to local Ollama when no key is set.",
  },
  {
    q: "Does it need an internet connection?",
    a: "Only for the one-time model download on first launch. After that it runs completely offline using local Ollama.",
  },
];
  ...
  return (
    <div className="relative mx-auto w-full max-w-[420px] select-none">
      ...
      {!visible && (
        ...
        >
          Press <kbd className="font-mono">Alt+A</kbd> to reopen ↑
        </div>
      )}

      <div className={`transition-all duration-150 ${visible ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none h-0 overflow-hidden"}`}>
        <div className={`relative rounded-2xl border bg-[#0d0b09] shadow-[0_32px_80px_rgba(0,0,0,0.8)] transition-all duration-100 ${flash ? "border-[var(--accent)] shadow-[0_0_24px_rgba(218,119,86,0.25)]" : "border-white/[0.08]"}`}>
          <div className="flex items-center gap-2 border-b border-white/[0.05] px-4 py-3">
            ...
            <span className="ml-auto flex items-center gap-2">
              ...
              <button onClick={() => { setVisible(false); setPhase("idle"); }} className="text-[#333] hover:text-[#888] transition-colors text-[11px] px-1">✕</button>
            </span>
          </div>
            ...
            <div className="flex gap-1.5">
              {DEMO_ACTIONS.map((a, i) => (
                <button
                  ...
                  className={`flex-1 rounded-lg py-2 text-xs font-semibold transition-all duration-150 ${active === i && phase !== "idle" ? "bg-[var(--accent)] text-white shadow-lg" : "bg-[#1a1714] text-[#555] hover:bg-[#221e1a] hover:text-[#999]"}`}
                >
                  {a.icon} {a.key}
                </button>
              ))}
            </div>
            <div className={`rounded-lg border bg-[#111009] transition-all duration-200 ${phase === "idle" ? "border-transparent opacity-0 h-0 overflow-hidden py-0" : "border-white/[0.05] opacity-100 px-3 py-3"}`}>
              {phase === "typing" && <p className="text-sm leading-relaxed text-[#c8c8c8]"><TypeWriter text={action.output} /></p>}
              {phase === "done" && (
                <>
                  <p className="text-sm leading-relaxed text-[#c8c8c8]">{action.output}</p>
                  <div className="mt-2.5 flex gap-1.5">
                    {["Copy", "Insert ↵", "Edit"].map(label => (
                      <button key={label} className="rounded px-2 py-1 text-[11px] text-[#444] hover:text-[#888] hover:bg-white/[0.03] transition-colors">{label}</button>
                    ))}
                  </div>
                </>
              )}
  ...
  return (
    <div className="border-b border-[var(--border)] last:border-0">
      ...
      >
        ...
        <span className={`shrink-0 text-[var(--muted)] transition-transform duration-200 ${open ? "rotate-45" : ""}`}>+</span>
      </button>
      {open && (
        <p className="pb-5 text-sm text-[var(--text-dim)] leading-relaxed">{a}</p>
      )}
    </div>
  );
}
...
function PlatformBtn({ label, icon, href, size = "lg", className = "" }: { label: string; icon: React.ReactNode; href: string; size?: "lg" | "sm"; className?: string }) {
  const [dl, setDl] = useState(false);
  const base = size === "lg" ? "rounded-xl px-7 py-3.5 text-sm font-semibold" : "rounded-lg px-4 py-2.5 text-sm font-medium";
  function handleClick() { setDl(true); setTimeout(() => setDl(false), 6000); }
  return (
    <a href={href} onClick={handleClick} className={`${base} inline-flex items-center gap-2 bg-[var(--accent)] text-white shadow-lg hover:bg-[var(--accent-dim)] active:scale-95 transition-all ${className}`}>
      {dl ? (<><span className="h-4 w-4 animate-spin rounded-full border-2 border-white/30 border-t-white shrink-0" />Downloading…</>) : (<>{icon}{DL_ICON}{label}</>)}
    </a>
  );
}
  ...
  return (
    <div className="fixed bottom-5 left-1/2 z-50 -translate-x-1/2 flex items-center gap-3 rounded-2xl border border-[var(--border)] bg-[var(--panel)] px-5 py-3.5 shadow-xl">
      ...
      <p className="text-sm text-[var(--text-dim)]">
        <span className="font-semibold text-[var(--text)]">{os} build</span> is being packaged — check back soon.
      </p>
  ...
  return (
    <main className="min-h-screen bg-[var(--surface)] text-[var(--text)] overflow-x-hidden">
      ...
      <nav className="sticky top-0 z-50 border-b border-[var(--border)] bg-[var(--surface)]/90 backdrop-blur-sm">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            <a href="#features"  className="hidden text-sm text-[var(--text-dim)] hover:text-[var(--text)] transition-colors md:block">Features</a>
            <a href="#use-cases" className="hidden text-sm text-[var(--text-dim)] hover:text-[var(--text)] transition-colors md:block">Use Cases</a>
            <a href="#faq"       className="hidden text-sm text-[var(--text-dim)] hover:text-[var(--text)] transition-colors md:block">FAQ</a>
            <a href="#download"  className="hidden text-sm text-[var(--text-dim)] hover:text-[var(--text)] transition-colors md:block">Download</a>
            <DownloadBtn size="sm" />
          </div>
        </div>
      </nav>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-6xl px-6 pt-28 pb-24 text-center">
          ...
          <h1 className="mx-auto max-w-4xl text-5xl font-semibold leading-[1.1] tracking-tight md:text-7xl">
            Your AI layer.{" "}<br className="hidden md:block" />
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
            <DownloadBtn size="lg" />
            <a href="#demo" className="inline-flex items-center gap-2 rounded-xl border border-[var(--border)] px-7 py-3.5 text-sm font-medium text-[var(--text-dim)] hover:text-[var(--text)] hover:border-[var(--muted)] transition-colors">
              See it live
          ...
          <p className="mt-3 text-xs text-[var(--muted)]">Free · No account needed · Windows &amp; macOS</p>
        </div>
      </section>
      ...
      <section id="features" className="mx-auto max-w-6xl px-6 py-28">
        <p className="mb-4 text-center text-xs font-semibold uppercase tracking-widest text-[var(--muted)]">Everything included</p>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {FEATURES.map(({ icon, title, body }) => (
            <div key={title} className="group rounded-2xl border border-[var(--border)] bg-[var(--panel)] p-7 hover:border-[var(--muted)] transition-colors">
      ...
      <section className="border-y border-[var(--border)] bg-[var(--panel)]">
        <div className="mx-auto max-w-6xl px-6 py-24">
          <p className="mb-16 text-center text-xs font-semibold uppercase tracking-widest text-[var(--muted)]">How it works</p>
          <div className="grid gap-6 md:grid-cols-3">
            {STEPS.map(({ n, title, body }) => (
              <div key={n} className="relative">
                ...
                {n !== "03" && <div className="absolute top-4 left-8 hidden h-px w-[calc(100%-2rem)] border-t border-dashed border-[var(--border)] md:block" />}
      ...
      {/* ── Use Cases ───────────────────────────────────────────────────────── */}
      <section id="use-cases" className="mx-auto max-w-6xl px-6 py-28">
        <p className="mb-4 text-center text-xs font-semibold uppercase tracking-widest text-[var(--muted)]">Built for your work</p>
        ...
        <p className="mb-16 text-center text-[var(--text-dim)] max-w-xl mx-auto">
          AI Cursor detects what you're working on and adjusts its prompts, tone, and actions to match your industry.
        </p>
        <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {USE_CASES.map(({ emoji, gradient, label, tagline, actions }) => (
            <div key={label} className="group rounded-2xl border border-[var(--border)] bg-[var(--panel)] overflow-hidden hover:border-[var(--muted)] transition-all hover:-translate-y-0.5">
              {/* illustration area */}
              <div className={`bg-gradient-to-br ${gradient} px-6 pt-8 pb-6 flex items-center gap-3`}>
                <span className="text-4xl leading-none">{emoji}</span>
                <div>
                  <p className="font-semibold text-[var(--text)]">{label}</p>
                  <p className="text-xs text-[var(--text-dim)] mt-0.5">{tagline}</p>
                </div>
              </div>
              {/* actions list */}
              <div className="px-6 py-4 border-t border-[var(--border)]">
                <ul className="space-y-1.5">
                  {actions.map(a => (
                    <li key={a} className="flex items-center gap-2 text-xs text-[var(--text-dim)]">
          ...
          <div className="grid gap-12 md:grid-cols-2 items-center">
            <div>
              ...
              <h2 className="text-3xl font-semibold tracking-tight mb-4">
                Your data stays<br /><span className="text-[var(--accent)]">on your machine.</span>
              </h2>
              <p className="text-[var(--text-dim)] leading-relaxed">
                AI Cursor runs entirely locally using qwen2.5:14b via Ollama. Style profiles, history, and preferences live in a folder on your machine.
                No analytics, no tracking, no cloud sync — unless you choose to add a cloud API key.
              </p>
            </div>
            <div className="grid gap-3">
              {[
                ["⊞", "Local model", "Runs qwen2.5:14b locally via Ollama — your text never leaves your machine"],
                ["◎", "No account", "Download, install, run. No sign-up, no email, no tracking."],
      ...
      <section id="faq" className="mx-auto max-w-3xl px-6 py-28">
        <p className="mb-4 text-center text-xs font-semibold uppercase tracking-widest text-[var(--muted)]">FAQ</p>
        <h2 className="mb-12 text-center text-3xl font-semibold tracking-tight">
          Common questions
        </h2>
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--panel)] divide-y divide-[var(--border)] px-8">
      ...
      <section id="download" className="mx-auto max-w-6xl px-6 py-32 text-center">
        <h2 className="text-4xl font-semibold tracking-tight md:text-5xl">Ready in 60 seconds.</h2>
        <p className="mt-4 text-lg text-[var(--text-dim)]">Download, install, press Alt+A.</p>

        <div className="mt-10 flex flex-col items-center gap-4">
          <DownloadBtn size="lg" className="shadow-[0_0_40px_rgba(218,119,86,0.2)]" />
          <p className="text-sm text-[var(--muted)]">
            Free · No account · Windows &amp; macOS · AI runs locally
          </p>
        </div>

        <div className="mx-auto mt-16 grid gap-6 max-w-2xl md:grid-cols-2">
          {/* Windows */}
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--panel)] p-6 text-left">
            <div className="flex items-center gap-2 mb-5">
              {WIN_ICON}
              <p className="text-xs font-semibold uppercase tracking-widest text-[var(--muted)]">Windows</p>
            </div>
            {[
              "Download AIcursor-windows-setup.exe",
              "Run the installer — no admin prompt needed",
              "AI Cursor appears in your Start Menu",
              "Press Alt+A on any text to start",
            ].map((step, i) => (
              <div key={i} className="flex items-start gap-3 mb-3 last:mb-0">
                <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[var(--panel-2)] font-mono text-[10px] text-[var(--muted)] mt-0.5">{i + 1}</span>
          ...
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--panel)] p-6 text-left">
            <div className="flex items-center gap-2 mb-5">
              {MAC_ICON}
              <p className="text-xs font-semibold uppercase tracking-widest text-[var(--muted)]">macOS</p>
            </div>
            {[
              "Download AIcursor-macos.dmg",
              "Open the DMG and drag AI Cursor to Applications",
              "Launch from Applications or Spotlight",
              "Press Option+A on any text to start",
            ].map((step, i) => (
              <div key={i} className="flex items-start gap-3 mb-3 last:mb-0">
                <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[var(--panel-2)] font-mono text-[10px] text-[var(--muted)] mt-0.5">{i + 1}</span>
                <p className="text-sm text-[var(--text-dim)]">{step}</p>
        ...
        <p className="mt-8 text-xs text-[var(--muted)]">
          On first launch, the AI model (~9 GB) downloads automatically with a progress bar. One-time only.
        </p>
      </section>
        ...
        <div className="mx-auto max-w-6xl px-6 py-8">
          <div className="flex flex-col items-center justify-between gap-4 md:flex-row">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-[var(--muted)]">v0.3.0</span>
            </div>
